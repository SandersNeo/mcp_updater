...
PowerShell helper `promote-existing-build.ps1` принимает `-UpdateLog` как optional override. Если `-UpdateLog` не указан, helper вычисляет resolved project root: `paths.root` из project config или директорию `project.json`, если `paths.root` отсутствует. Затем helper выбирает самый поздний по timestamp в имени `YYYYMMDD-HHMMSS-update.log` из `<resolved-root>/logs`, чтобы извлечь `Target commit`, `Source fingerprint` и `Report hash`.
