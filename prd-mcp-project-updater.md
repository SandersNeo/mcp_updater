...
Index storage больше не выводится из `paths.root`.

## 5. MCP Index Storage

Каждый project config обязан задавать:
...
6. Если commit/fingerprint/report/storage не изменились и нет `--force`, пропустить update.
...
9. Запустить новый production container.
...
- запускает production container на rollback state;
- выполняет production smoke-test;
...
- config требует `mcp.indexStorageRoot`;
